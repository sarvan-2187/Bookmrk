    ...
    >
      ...
      >
        <div className={`px-5 py-4 border-b ${isNeoBrutalistMode ? 'border-black' : isDark ? 'border-zinc-800' : 'border-zinc-200'}`}>
          <h3 className={`text-lg font-semibold ${isNeoBrutalistMode ? 'text-black' : isDark ? 'text-zinc-100' : 'text-zinc-900'}`}>Page Settings</h3>
          <p className={`mt-1 text-sm ${isNeoBrutalistMode ? 'text-black/70' : isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>Manage {pageName}.</p>
        </div>

        <div className="px-5 py-5">
          <section className="space-y-3 mb-6">
            <div className="flex items-center justify-between">
              <h4 className={`text-sm font-semibold ${isNeoBrutalistMode ? 'text-black' : isDark ? 'text-zinc-200' : 'text-zinc-800'}`}>Edit page name</h4>
          ...
          <section className="space-y-3 mb-6">
            <div className="flex items-center justify-between">
              <h4 className={`text-sm font-semibold ${isNeoBrutalistMode ? 'text-black' : isDark ? 'text-zinc-200' : 'text-zinc-800'}`}>Bookmarks per board</h4>
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <div className={`flex-1 text-sm ${isNeoBrutalistMode ? 'text-black/70' : 'text-zinc-500'}`}>Show this many bookmarks per board on the page; the rest will be hidden under a "Show more" button.</div>
              <button
                type="button"
                disabled={!onSaveVisibleCount}
                onClick={() => onSaveVisibleCount && onSaveVisibleCount(visible)}
                className={`inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition-colors ${onSaveVisibleCount ? isNeoBrutalistMode ? 'border-2 border-black bg-black text-white shadow-[2px_2px_0_#000] hover:bg-zinc-900' : isDark ? 'bg-zinc-100 text-zinc-900 hover:bg-zinc-200' : 'bg-zinc-900 text-zinc-100 hover:bg-zinc-800' : 'bg-zinc-300/50 text-zinc-500 cursor-not-allowed'}`}
              >
                <Save className="h-4 w-4" />
                <span>Save</span>
              </button>
