...
export function BookmarkTile({ bookmark, boardId, pageId, isOverlay }: BookmarkTileProps) {
  const { deleteBookmark, dragMode, addToast, data, blurMode, tempUnblurBookmarkId, showTemporaryUnblur, clearTemporaryUnblur } = useStore();
  ...
  const handleMouseEnter = () => {
    if (!blurMode || !showTemporaryUnblur || dragMode) return;
    try {
      const el = elRef.current;
      if (el) {
        const rect = el.getBoundingClientRect();
        showTemporaryUnblur(bookmark.id, { left: rect.left, top: rect.top, width: rect.width, height: rect.height });
      }
    } catch (e) {
      // ignore
    }
  ...
  const handleMouseLeave = () => {
    if (!blurMode || !clearTemporaryUnblur) return;
    clearTemporaryUnblur();
  };

  const handleOpen = () => {
    if (blurMode) {
      // while blur is active, don't allow opening bookmarks
      addToast('Disable Blur to open bookmarks', 'info');
      if (blurMode && showTemporaryUnblur) {
        try {
          const el = elRef.current;
          if (el) {
            const rect = el.getBoundingClientRect();
    ...
    // Normal open when not blurred
    window.open(bookmark.url, openInNewTab ? '_blank' : '_self');
  };

  const isRevealed = Boolean(blurMode && tempUnblurBookmarkId === bookmark.id);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isRevealed ? 'bg-transparent border border-transparent' : (isDark ? "hover:bg-white/5 border border-transparent hover:border-white/5" : "hover:bg-zinc-100 border border-transparent hover:border-zinc-200"),
    ...
    >
      ...
      <div className="flex-1 min-w-0 pr-8">
        <h4 className={`font-medium truncate transition-colors ${compactMode ? 'text-[11px]' : 'text-[13px]'} ${isNeoBrutalistMode ? 'text-black' : isDark ? 'text-zinc-200' : 'text-zinc-800'} ${isRevealed ? '' : (isNeoBrutalistMode ? 'group-hover:text-black' : isDark ? 'group-hover:text-white' : 'group-hover:text-zinc-950')}`} title={bookmark.title}>{truncatedTitle}</h4>
        {showDescriptions && !!(bookmark.description || bookmark.note) && (
          <p className={`truncate mt-0.5 font-medium tracking-wide transition-colors ${compactMode ? 'text-[9px]' : 'text-[10px]'} ${isNeoBrutalistMode ? 'text-black/80' : isDark ? 'text-zinc-500' : 'text-zinc-500'} ${isRevealed ? '' : (isNeoBrutalistMode ? 'group-hover:text-black' : isDark ? 'group-hover:text-zinc-400' : 'group-hover:text-zinc-700')}`}>
            {bookmark.description || bookmark.note}
          </p>
        )}
      </div>

      <div className={cn("absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 transition-all transform", isRevealed ? 'opacity-0 translate-x-2' : 'opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0')}>
        <button
          ...
          className={cn('p-1.5 focus:outline-none rounded-lg transition-all', isRevealed ? (isNeoBrutalistMode ? 'text-black' : isDark ? 'text-zinc-500' : 'text-zinc-500') : (isNeoBrutalistMode ? 'text-black hover:text-black hover:bg-black/10' : isDark ? 'text-zinc-500 hover:text-zinc-100 hover:bg-white/10' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100'))}
