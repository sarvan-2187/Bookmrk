...
import { EyeOff, PanelLeft, Plus, Search, Move, Settings, Palette, Check } from 'lucide-react';
import { useStore } from '../store/useStore';

type ToolbarProps = {
  onOpenSearch: () => void;
  onAddPage?: () => void;
  layoutMode?: 'discord' | 'simple' | 'neobrutalist';
};

export function Toolbar({ onOpenSearch, onAddPage, layoutMode = 'discord' }: ToolbarProps) {
  const { toggleSidebar, toggleBlurMode, toggleDragMode, data, activePageId, setActivePage, openQuickSave, openSettingsModal, dragMode, addToast, updateSettings } = useStore();
  ...
  const handleThemeSelect = (nextTheme: 'discord' | 'simple' | 'neobrutalist') => {
    updateSettings({ themeMode: nextTheme });
    setThemeMenuOpen(false);
  };

  return (
    <header className={`h-14 border-b flex items-center px-4 justify-between shrink-0 z-20 ${isNeoBrutalistMode ? 'border-black bg-white shadow-[4px_4px_0_#000]' : isCompactNavMode ? (isDark ? 'border-white/5 bg-zinc-950/10' : 'border-zinc-200 bg-white/60 backdrop-blur-sm') : isDark ? 'border-white/5 bg-zinc-950/20 backdrop-blur-sm' : 'border-zinc-200 bg-white/70 backdrop-blur-sm shadow-[0_8px_24px_rgba(0,0,0,0.04)]'}`}>
          ...
          <div className="flex min-w-0 items-center gap-2 overflow-x-auto py-1">
            {data.pages.map((page) => (
              <button
                ...
                className={`shrink-0 inline-flex h-10 min-w-[7rem] items-center justify-center rounded-xl border-2 px-4 text-sm font-medium transition-colors ${
                ...
                title={page.name}
              >
                <span className="max-w-[9rem] truncate">{page.name}</span>
              </button>
            ))}
            ...
            <button
              type="button"
              onClick={onAddPage}
              className={`shrink-0 inline-flex h-10 w-10 items-center justify-center rounded-xl border-2 text-sm font-semibold transition-colors ${isNeoBrutalistMode ? 'border-black bg-white text-black shadow-[4px_4px_0_#000] hover:bg-zinc-100' : isDark ? 'border-white/10 bg-white/5 text-zinc-200 hover:bg-white/10' : 'border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-100'}`}
              title="Add page"
            >
      ...
      <div className="flex shrink-0 items-center gap-2">
        <button
          ...
          disabled={dragMode}
          className={`p-1.5 rounded-md transition-colors ${dragMode ? 'opacity-50 cursor-not-allowed' : isNeoBrutalistMode ? 'border-2 border-black bg-white text-black hover:bg-zinc-100 shadow-[3px_3px_0_#000]' : isDark ? 'text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100'}`}
          title="Quick Save"
        >
